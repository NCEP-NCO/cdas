...
C   99-12-06  EBISUZAKI   THIS CODE IS NEVER GETS CALLED AND IS
C                         NEEDS WORK TO GET COMPLEX PACKING WORKING
C
...
c
c initialized
c	mpts, const, iberr, iw, fwork, iwork
C
C
...
C                     PRINT *,'MINIMA 400',I,IWORK(I),MPTS
C                 END IF
...
C    &              10(3X,10I10,/))') (IWORK(I),I=MPTS-5,MPTS)
C
...
C             WRITE (6,FMT='(''  END OF ARRAY  = '',/,
C    &              10(3X,10F8.2,/))') (FWORK(I),I=MPTS-5,MPTS)
...
C    &                  10(3X,10I6,/))') (IWORK(I),I=1,MPTS)
C
               CALL W3FI82 (IWORK,FVAL1,FDIFF1,MPTS,PDS,IGDS)
C
...
C                        BIT MAP. ERGO ROW BY ROW - COL BY COL
              CALL FI7503 (IWORK,IPFLD,MPTS,IBDSFL,BDS11,
...
C    &                  10(3X,10I6,/))') (IWORK(I),I=1,MPTS)
C         WRITE (6,FMT='(''  END OF ARRAY = '',/,
C    &                  10(3X,10I6,/))') (IWORK(I),I=MPTS-5,MPTS)
C         PRINT*,' REFNCE,ISCAL2, KWIDE AT CALL TO FI7501',
C    &             REFNCE, ISCAL2,KWIDE
C
C                         SECOND ORDER PACKING
C
          CALL FI7501 (IWORK,IPFLD,MPTS,IBDSFL,BDS11,
...
C
              CALL W3FI58(IWORK,MPTS,IWORK,PFLD,NBITS,LEN,KMIN)
              RMIN   = KMIN
              REFNCE  = RMIN
              ISCALE = 0
...
C
              CALL W3FI59(FWORK,MPTS,IBITL,IWORK,PFLD,ISCALE,LEN,RMIN)
              REFNCE = RMIN
...
C
      RETURN
      END
      SUBROUTINE FI7501 (IWORK,IPFLD,MPTS,IBDSFL,BDS11,
     *           LEN,LENBDS,PDS,REFNCE,ISCAL2,KWIDE)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
...
C                         OF AN OFFSET.
C
C USAGE:    CALL FI7501 (IWORK,IPFLD,MPTS,IBDSFL,BDS11,
C    *           LEN,LENBDS,PDS,REFNCE,ISCAL2,KWIDE)
C   INPUT ARGUMENT LIST:
C     IWORK    - INTEGER SOURCE ARRAY
C     MPTS     - NUMBER OF POINTS IN IWORK
C     IBDSFL   - FLAGS
C
...
C     PRINT *,'NEXT ITERATION STARTS AT',ISTART
       IF (ISTART.GT.MPTS) THEN
           GO TO 4000
       ELSE IF (ISTART.EQ.MPTS) THEN
           KPTS    = 1
           MXDIFF  = 0
           GO TO 2200
       END IF
...
C                     LOOK FOR REPITITIONS OF A SINGLE VALUE
       CALL FI7502 (IWORK,ISTART,MPTS,ISAME)
       IF (ISAME.GE.15) THEN
           KOUNT = KOUNT + 1
...
C                     LOOK FOR SETS OF VALUES IN TREND SELECTED RANGE
           CALL FI7513 (IWORK,ISTART,MPTS,NMAX,NMIN,INRNGE)
...
C
      RETURN
      END
      SUBROUTINE FI7502 (IWORK,ISTART,MPTS,ISAME)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
...
C
C USAGE:    CALL FI7502 (IWORK,ISTART,MPTS,ISAME)
C   INPUT ARGUMENT LIST:
...
C  -------------------------------------------------------------
  ...
  100 CONTINUE
      RETURN
      END
      SUBROUTINE FI7503 (IWORK,IPFLD,MPTS,IBDSFL,BDS11,
     *           LEN,LENBDS,PDS,REFNCE,ISCAL2,KWIDE,IGDS)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
...
C   93-08-06  CAVANAUGH
C
C USAGE:    CALL FI7503 (IWORK,IPFLD,MPTS,IBDSFL,BDS11,
...
C
 ...
 1000 CONTINUE
      RETURN
      END
      SUBROUTINE FI7513 (IWORK,ISTART,MPTS,IMAX,IMIN,INRNGE)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
...
C
C USAGE:    CALL FI7513 (IWORK,ISTART,MPTS,IMAX,IMIN,INRNGE)
C   INPUT ARGUMENT LIST:
...
C
C$$$
      INTEGER        IWORK(*),MPTS,ISTART,INRNGE,INRNGA,INRNGB
      INTEGER        IMAX,IMIN,MXVAL,MAXB,MINB,MXVALB
      INTEGER        IBITS(31)
C
      SAVE
C
...
C
C                     GET BLOCK A
      CALL FI7516 (IWORK,MPTS,INRNGA,ISTRTA,
     *                                  IMAX,IMIN,MXVAL,LWIDE)
C  ********************************************************************
C
      ISTRTB  = ISTRTA + INRNGA
 2000 CONTINUE
C                         IF HAVE PROCESSED ALL DATA, RETURN
      IF (ISTRTB.GT.MPTS) THEN
C                         NO MORE DATA TO LOOK AT
          INRNGE  = INRNGA
          RETURN
      END IF
C                     GET BLOCK B
      CALL FI7502 (IWORK,ISTRTB,MPTS,ISAME)
      IF (ISAME.GE.15) THEN
...
C
      ISTRTB  = ISTRTA + INRNGA
      CALL FI7516 (IWORK,MPTS,INRNGB,ISTRTB,
...
C                            USES FEWER BITS
C
          CALL FI7518 (*8000,IWORK,MPTS,ISTRTA,INRNGA,INRNGB,
     *                          IMAX,IMIN,LWIDE,MXVAL)
...
C                            USES FEWER BITS
C
          CALL FI7517 (*8000,IWORK,MPTS,ISTRTB,INRNGA,
...
C                           GET NEXT BLOCK A
 9000 CONTINUE
      RETURN
      END
      SUBROUTINE FI7516 (IWORK,MPTS,INRNG,ISTART,IMAX,IMIN,MXVAL,LWIDTH)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
...
C   YY-MM-DD  MODIFIER1   DESCRIPTION OF CHANGE
C
C USAGE:    CALL FI7516 (IWORK,MPTS,INRNG,ISTART,IMAX,IMIN,MXVAL,LWIDTH)
C   INPUT ARGUMENT LIST:
C     *        - RETURN ADDRESS IF ENCOUNTER SET OF SAME VALUES
C     IWORK    - DATA ARRAY
C     MPTS     - NUMBER OF POINTS IN DATA ARRAY
C     ISTART   - STARTING LOCATION IN DATA
C
C   OUTPUT ARGUMENT LIST:      (INCLUDING WORK ARRAYS)
C     INRNG    - NUMBER OF POINTS SELECTED
C     IMAX      - MAXIMUM VALUE OF POINTS
C     IMIN      - MINIMUM VALUE OF POINTS
...
C
C$$$
      INTEGER        IWORK(*),MPTS,ISTART,INRNG,IMAX,IMIN,LWIDTH,MXVAL
      INTEGER        IBITS(31)
C
      SAVE
C
...
C
      ...
      DO 1000 I = ISTART+1, JQ
          CALL FI7502 (IWORK,I,MPTS,ISAME)
          IF (ISAME.GE.15) THEN
              GO TO 5000
          END IF
          INRNG  = INRNG + 1
...
C             PRINT *,'RETURNED',INRNG,' VALUES'
 ...
 9000 CONTINUE
      RETURN
      END
      SUBROUTINE FI7517 (*,IWORK,MPTS,ISTRTB,INRNGA,
     *                           MAXB,MINB,MXVALB,LWIDEB)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
...
C   94-01-21  CAVANAUGH
C
C USAGE:    CALL FI7517 (*,IWORK,MPTS,ISTRTB,INRNGA,
...
C
C$$$
      INTEGER        IWORK(*),MPTS,ISTRTB,INRNGA
...
C
 9000 CONTINUE
      RETURN
      END
      SUBROUTINE FI7518 (*,IWORK,MPTS,ISTRTA,INRNGA,INRNGB,
...
C   YY-MM-DD  MODIFIER1   DESCRIPTION OF CHANGE
C
C USAGE:    CALL FI7518 (IWORK,MPTS,ISTRTA,INRNGA,ISTRTB,INRNGB,
...
C
C$$$
      INTEGER        IWORK(*),MPTS,ISTRTA,INRNGA
