...
c        first byte
         ...
         itmp = and(mova2i(in(index)),ones(8-ibit))
...
c        now transfer whole bytes
         do while (bitcnt.ge.8)
             itmp = or(ishft(itmp,8),mova2i(in(index)))
             bitcnt = bitcnt - 8
             index = index + 1
         enddo
...
c        get data from last byte
         if (bitcnt.gt.0) then
             itmp = or(ishft(itmp,bitcnt), and(ishft(mova2i(in(index)),
...
c        make byte aligned 
         if (ibit.ne.7) then
             ...
             itmp2 = and(ishft(itmp,7-ibit),imask)
             itmp3 = and(mova2i(out(index)), 255-imask)
...
c        do last byte
         ...
         if (bitcnt.gt.0) then
             itmp2 = and(itmp,ones(bitcnt))
             itmp3 = and(mova2i(out(index)), 255-ones(bitcnt))
             out(index) = char(or(itmp2,itmp3))
         endif
      enddo
