...
C
      ...
      ELSE IF (IPFLAG .EQ. 1) THEN
        IF (AND(MOVA2I(PDS(8)),64) .EQ. 64) THEN
C         BOTH GDS AND BMS
          PDS(8) = CHAR(192)
...
C
      ...
      IF (AND(JSCALE,32768).NE.0) THEN
        JSCALE = - AND(JSCALE,32767)
      END IF
      SCALE  = 10.0 ** JSCALE
      IF (ITYPE .EQ. 0) THEN
        DO I = 1,NPTS
...
C
     ...
     &         NPTS,BDS11,PFLD,PFLD,LEN,LENBDS,IBERR,PDS,IGDS)
        IF (IBERR .EQ. 1) THEN
          JERR = 3
          GO TO 900
        END IF
...
C     THIS IS BOTH COMPUTER AND COMPILER DEPENDENT, 
C
