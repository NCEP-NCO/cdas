...
C  TRANSFORM TO PRESSURE GRIB AND ATTEMPT TO READ NEXT SIGMA HEADER
     ...
     &          '   PRESSURE GRID ',IO,'*',JO,'*',KO
        ...
        call baopen(npgb,trim(pgbnam),iret)
        if (iret.ne.0) then
...
c     ITL(KPU:KPU+KO-1)=IPO
	call ifill(itl,kpu,kpu+ko-1,ipo)
c     ITL(KPV:KPV+KO-1)=IPO
	call ifill(itl,kpv,kpv+ko-1,ipo)
c     ITL(KPO:KPO+KO-1)=IPO
	call ifill(itl,kpo,kpo+ko-1,ipo)
c     ITL(KPZ:KPZ+KO-1)=IPO
	call ifill(itl,kpz,kpz+ko-1,ipo)
c     ITL(KPT:KPT+KO-1)=IPO
	call ifill(itl,kpt,kpt+ko-1,ipo)
c     ITL(KPR:KPR+KO-1)=IPO
	call ifill(itl,kpr,kpr+ko-1,ipo)
c     ITL(KPA:KPA+KO-1)=IPO
	call ifill(itl,kpa,kpa+ko-1,ipo)
...
c     IL1(KPTU:KPTU+KT-1)=NPT1
	call ifill(il1,kptu,kptu+kt-1,npt1)

c     IL1(KPTV:KPTV+KT-1)=NPT1
	call ifill(IL1,KPTV,KPTV+KT-1,NPT1)
c     IL1(KPTT:KPTT+KT-1)=NPT1
	call ifill(IL1,KPTT,KPTT+KT-1,NPT1)
c     IL1(KPTR:KPTR+KT-1)=NPT1
	call ifill(IL1,KPTR,KPTR+KT-1,NPT1)
